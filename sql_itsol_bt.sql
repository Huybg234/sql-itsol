...
insert into subject values (2, 'Trí tuệ nhân tạo', 45);
insert into subject values (3, 'Truyền tin', 45);
insert into subject values (4, 'Đồ họa', 60);
insert into subject values (5, 'Văn phạm', 45);


-- faculty
insert into faculty values (1, 'Anh - Văn');
insert into faculty values (2, 'Tin học');
insert into faculty values (3, 'Triết học');
insert into faculty values (4, 'Vật lý');


-- student
insert into student values (1, 'Nguyễn Thị Hải', 'Nữ', to_date('19900223', 'YYYYMMDD'), 'Hà Nội', 130000, 2);
insert into student values (2, 'Trần Văn Chính', 'Nam', to_date('19921224', 'YYYYMMDD'), 'Bình Định', 150000, 4);
insert into student values (3, 'Lê Thu Yến', 'Nữ', to_date('19900221', 'YYYYMMDD'), 'TP HCM', 150000, 2);
insert into student values (4, 'Lê Thu Yến', 'Nữ', to_date('19900221', 'YYYYMMDD'), 'TP HCM', 170000, 2);
insert into student values (5, 'Trần Anh Tuấn', 'Nam', to_date('19901220', 'YYYYMMDD'), 'Hà Nội', 180000, 1);
insert into student values (6, 'Trần Thanh Mai', 'Nữ', to_date('19910812', 'YYYYMMDD'), 'Hải Phòng', null, 3);
insert into student values (7, 'Trần Thị Thu Thủy', 'Nữ', to_date('19910102', 'YYYYMMDD'), 'Hải Phòng', 10000, 1);
...
select student.scholarship from student where student.scholarship > 100000 order by faculty_id desc;
-- 8. Sinh viên có học bổng từ 150000 trở lên và sinh ở Hà Nội
select student.scholarship,student.hometown from student where student.scholarship > 150000 and hometown = 'Hà Nội';
-- 9. Những sinh viên có ngày sinh từ ngày 01/01/1991 đến ngày 05/06/1992
select * from student where student.birthday between to_date('01/01/1991','DD/MM/YYYY') and to_date('05/06/1992','DD/MM/YYYY');
...
select count(id) as student_total from student; 
-- 3. Tính tổng số sinh viên nam và tổng số sinh viên nữ.
select gender, count(id) from student group by student.gender;
-- 4. Tính tổng số sinh viên từng khoa (chưa cần JOIN)
select faculty.name, count(student.id) from student,faculty where student.faculty_id = faculty.id group by faculty.name;
...
select faculty.name, count(student.scholarship) from faculty, student where faculty.id = student.faculty_id group by faculty.name;
-- 8. Cho biết học bổng cao nhất của mỗi khoa
select faculty.name, max(student.scholarship) from faculty, student where faculty.id = student.faculty_id group by faculty.name;
...
select student.hometown, count(student.id) as total from student group by hometown having count(student.id) > 2;
...
where exam_management.number_of_exam_taking = 1 and exam_management.mark <= 4 and student.id = exam_management.student_id group by student.name;
...
select student.name from student where to_char(birthday,'MM')='02' and student.hometown = 'Hà Nội';
...
select student.name, count(subject_id) from exam_management
